"""
train_moe_stage1b.py -- Stage 1b: hidden-state routed MoE

Stage 1 routed on the true residual direction (v_target - v_base). That gave a
large, stable training gain (val 0.32 to 0.235), but the residual needs the
ground-truth clean image, which doesn't exist at generation. 

The router reads the layer-l hidden state h_l, which is available
at every denoising step (it only needs the noisy latent and t). The residual is
kept only as the supervision target and add a distillation cross-entropy pulling
the hidden-state router's decision toward that residual-optimal choice.

So at inference, routing depends only on h_l: no oracle and val and FID should
both hold because nothing the router needs disappears at generation.
"""
...
def main():
    ...
    # capture the layer-l hidden state on the frozen base. Because layers 0..l-1
    # are frozen and adapter-free, this hidden state is identical to the one the
    # MoE model sees at layer l and it is available at inference
    ...
    # These define the residual-optimal assignment: a token's target expert is the
    # centroid closest to its true residual direction. That is the routing Stage 1
    # made with the true residual, now used only to supervise.
    ...
    for epoch in range(args.epochs):
        ...
        for step, (latents, text_emb, _lbl) in enumerate(tqdm(train_dl, desc=f"E{epoch+1}")):
            ...
            with torch.no_grad():
                ...
                target_expert = sim.argmax(-1)  # [B,T] in [0, n_experts)

            with torch.amp.autocast("cuda"):
                # The router reads the layer-l hidden state, available at
                # inference. It is produced by layers 0..l-1, which are frozen and
                # adapter-free, so it is identical in the base pass and the MoE
                # pass (the adapter only modifies the FF output, not its input).
                ...
                # distillation: hidden-state router logits toward the residual-optimal target
